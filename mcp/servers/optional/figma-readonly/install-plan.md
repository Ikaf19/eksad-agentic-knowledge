# Install Plan — Figma Read-only MCP
...
```text
/opt/eksad-mcp/bin/figma-readonly
```
...
- `FIGMA_READONLY_TOKEN` — required=true, secret=true: Read-only design token.
...
```bash
python3 mcp/scripts/render-hermes-config.py figma-readonly
```
