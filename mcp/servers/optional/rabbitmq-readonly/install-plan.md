# Install Plan — RabbitMQ Read-only MCP
...
```text
/opt/eksad-mcp/bin/rabbitmq-readonly
```
...
- `RABBITMQ_READONLY_URL` — required=true, secret=true: Read-only management endpoint/credential.
...
```bash
python3 mcp/scripts/render-hermes-config.py rabbitmq-readonly
```
