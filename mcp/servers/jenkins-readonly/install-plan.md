# Install Plan — Jenkins Read-only MCP
...
```text
/opt/eksad-mcp/bin/jenkins-readonly
```
...
- `JENKINS_BASE_URL` — required=true, secret=false: Jenkins base URL.
- `JENKINS_READONLY_TOKEN` — required=true, secret=true: Read-only Jenkins token/API key.
...
```bash
python3 mcp/scripts/render-hermes-config.py jenkins-readonly
```
