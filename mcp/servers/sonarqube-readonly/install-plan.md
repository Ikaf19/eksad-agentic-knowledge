# Install Plan — SonarQube Read-only MCP
...
```text
/opt/eksad-mcp/bin/sonarqube-readonly
```
...
- `SONARQUBE_BASE_URL` — required=true, secret=false: SonarQube base URL.
- `SONARQUBE_READONLY_TOKEN` — required=true, secret=true: Read-only token.
...
```bash
python3 mcp/scripts/render-hermes-config.py sonarqube-readonly
```
