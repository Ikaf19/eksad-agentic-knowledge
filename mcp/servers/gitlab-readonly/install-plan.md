# Install Plan — GitLab Read-only MCP
...
```text
/opt/eksad-mcp/bin/gitlab-readonly
```
...
- `GITLAB_READONLY_TOKEN` — required=true, secret=true: Read-only GitLab token.
- `GITLAB_BASE_URL` — required=true, secret=false: GitLab CE/Cloud base URL.
...
```bash
python3 mcp/scripts/render-hermes-config.py gitlab-readonly
```
